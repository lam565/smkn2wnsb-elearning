<?php
mysqli_query($connection, "UPDATE lihat SET lihat='0' WHERE user_lihat='$_SESSION[username]' AND apa_lihat='like'");
...
?>
<title><?php echo $datauseruser["username"];?> | </title>
<?php
    ...
    <div id="page-inner">
        <div class="row">
            ...
            <div>
                ...
                $querykomentarnya = mysqli_query($connection, "SELECT*FROM suka_post WHERE penulis_post='$data2[username]' AND user_suka !='$_SESSION[username]' ORDER BY id_suka DESC LIMIT $posisi, $batas");
                ...
                while ($datakomennya = mysqli_fetch_array($querykomentarnya)) {
                    $datakomentar = mysqli_fetch_array(mysqli_query($connection, "SELECT*FROM login WHERE username='$datakomennya[user_suka]'"));
        ...
        ?>@<?php echo $datakomennya["user_suka"];?></a></b> Ngasih Jempol Pada Postingan <b><a href="./?p=post&id=<?php echo $datapostnya["id_post"];?>&post_by=<?php echo $datapostnya["penulis_post"];?>"><?php echo $judulpostnya;?></a></b> Kamu</div>
                ...
                $hitung2 = mysqli_num_rows(mysqli_query($connection, "SELECT*FROM suka_post WHERE penulis_post='$_SESSION[username]'"));
                ...
                for ($i=1; $i <= $bagi ; $i++) { 
                    if ($hal==$i) {
                                    ...
                                    echo "<a href='./?p=jempol&post_user=$_SESSION[username]&hal=$i'><span class='badge'>$i</span></a> ";
                                }
                }
                mysqli_query($connection, "UPDATE suka_post SET lihat_suka='0' WHERE penulis_post='$_SESSION[username]'");
                ?>
            </div>
            </div>
        </div>
...
echo "<script>window.location='./?p=jempol&post_user=$_SESSION[username]';</script>";
}
            ?>